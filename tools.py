...
def fourier_analysis(video_loader, pixel_size, x_up=0, x_down=-1, y_left=0, y_right=-1,start_frame=0, end_frame=100,detrend=False):
    ...
    frequencies, magnitudes = calculate_wavenumbers_1D(video_loader,start_frame,end_frame,x_slice,y_slice,pixel_size,detrend=detrend)
...
def fourier_animation(frequencies, magnitudes, baseline_horizontal=0, baseline_vertical=0, average=20, fit_min=0.1, fit_max=0.5, n_max_mag=10):
    ...
    def update(frame_index):
        ...
        horizontal_fit_params = gaussian_fit(frequencies["horizontal"][frame_index], precomputed_differences["horizontal_diff"][frame_index], fit_min, fit_max)
        ...
        vertical_line.set_label(f"Weighted Average (wavenumber): {weighted_avg:.3f} wavelenght (mm): {1/weighted_avg:.3f}")
        ## plot only if the fit is successful
        if horizontal_fit_params is not None:
            horizontal_gaussian_line.set_label(f"Amplitude: {horizontal_fit_params[0]:.3f}, Mean: {horizontal_fit_params[1]:.3f}, Std: {horizontal_fit_params[2]:.3f}, Wavelength (mm) {1/horizontal_fit_params[1]:.3f}")
            horizontal_gaussian_line.set_data(frequencies["horizontal"][frame_index], horizontal_fit_params[0] * np.exp(-0.5 * ((frequencies["horizontal"][frame_index] - horizontal_fit_params[1]) / horizontal_fit_params[2]) ** 2))
